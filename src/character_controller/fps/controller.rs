...
pub struct CameraConfig {
    /// The height offset from the top of the player to eye level.
    pub height_offset: f32,
    /// The offset from the player's center to the camera. Can be used for screen shake.
    pub offset: Vec3,
    /// The angle offset from the player's forward direction.
    pub angle_offset: Vec3,
}

impl CameraConfig {
    pub fn new(height_offset: f32) -> Self {
        Self {
            height_offset,
            offset: Vec3::ZERO,
            angle_offset: Vec3::ZERO,
        }
    }
}
...
pub struct FpsController {
    ...
    traction_normal_cutoff: f32,
    friction_speed_cutoff: f32,
    jump_speed: f32,
    fly_speed: f32,
    crouched_speed: f32,
    crouch_speed: f32,
...
impl FpsController {
    pub fn new(upright_height: f32, crouch_height: f32) -> Self {
        Self {
            upright_height,
            crouch_height,
            ..Default::default()
        ...
        query.iter_mut()
    {
        match controller.move_mode {
            ...
            MoveMode::Ground => {
                ...
                controller.is_crouched = input.crouch;
                let max_speed = if controller.is_crouched {
                ...
                if let Some((hit, hit_details)) = unwrap_hit_details(ground_cast) {
                    ...
                    controller.is_grounded = has_traction;
